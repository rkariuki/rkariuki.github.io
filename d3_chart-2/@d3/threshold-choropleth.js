...
function extract_number_yea(text){
	if (text != null){
...
//extract total nay from title
function extract_number_nay(text){
	if (text != null){
		var text_str = text.split(",");
		if (text_str.length > 3){
			var total_nay = text_str[3];
			var total_nay_arr = total_nay.split(":");
			if (total_nay_arr.length > 1){
				return parseFloat(total_nay_arr[1].trim());
			}			
		}		
	}
...
md``
  ...
  main.variable(observer("chart")).define("chart", ["d3","DOM","topojson","us","color1","color2","data","format"], function(d3,DOM,topojson,us,color1,color2,data,format)
{
  const width = 960;
  const height = 600;
  ...
  svg.append("g")
    ...
    .join("path")
	  ...
	  d => ((extract_number_yea(data.get(d.id)) > extract_number_nay(data.get(d.id)))?color1(extract_number_yea(data.get(d.id))):color2(extract_number_nay(data.get(d.id))))
...
Object.assign(new Map(await d3.csv("../d3_chart/Billiard2.csv", ({Name,id,Total_YEA,Total_NAY}) =>
 [id, Name + ', Total yea: ' + Total_YEA + ', Total nay: ' + Total_NAY])), {title: ""})
  ...
  main.variable(observer("color1")).define("color1", ["d3"], function(d3){
  ...
  main.variable(observer("color2")).define("color2", ["d3"], function(d3){
